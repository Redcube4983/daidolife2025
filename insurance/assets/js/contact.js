...
function kankeiCheck() {
    // ページ読み込み時 チェック状態を確認
    var kankei = $('input[name="kankei"]:checked').val();
    var kankei_namae = $('input[name="kankei_namae"]');
    var kankei_namae_kana = $('input[name="kankei_namae_kana"]');

    if(kankei == "本人" || kankei == undefined) {
        kankei_namae.prop("disabled", true);
        kankei_namae_kana.prop("disabled", true);
    } else {
        kankei_namae.prop("disabled", false);
        kankei_namae_kana.prop("disabled", false);
    }
    // 確認画面用
    $('.form-confirm input').each(function(){
...
function check_file_type(){
    if($('input[name="file_type"]').length){
        var selectedValue = $('input[name="file_type"]:checked').val(); // 選択されたラジオボタンの値
        var nosyaSelect = $('#nosya_m');  // <select> 要素
        
        // 条件に応じて <select> を制御
        if (selectedValue === 'ご家族でお車を増やす、または新規取得') {
            $('.form-nosya-m').show();
            nosyaSelect.prop('disabled', false); // 選択可能
        } else {
            nosyaSelect.prop('disabled', true);  // 選択不可
            $('.form-nosya-m').hide();
        }
    }
}
...
if($('input[name="introducer"]').length){
    $('input[name="introducer"]').on('change', function () {
        check_introducer();
    });
}

function check_introducer(){
    if($('input[name="introducer"]').length){
        var introducer_value = $('input[name="introducer"]:checked').val(); // 選択されたラジオボタンの値
        var name_full_input = $('#name_full');
        
        // 条件に応じて <select> を制御
        if (introducer_value === 'あり') {
            $('.form-introducer-ari').show();
            name_full_input.prop('disabled', false);
        } else {
            name_full_input.prop('disabled', true);
            $('.form-introducer-ari').hide();
        }
    }
}
...
submit_btn.on('click',function(){
    ...
    var nosya_m = $('select[name="nosya_m"]');
    var introducer = $('input[name="introducer"]:checked').val();
    var name_full = $('input[name="name_full"]');
...
//     // バリデーションチェックパターン
    ...
    if($('#form-checkList-kankei').length){
        ...
        if(kankei == "本人" || kankei == undefined) {
            success($(kankei_namae.next('p.error_txt')));
            success($(kankei_namae_kana.next('p.error_txt')));
        } else {
            kankei_namae.next('p.error_txt').text("お名前を入力してください。");
            error_text(kankei_namae);
            if(kankei_namae_kana.val() !== "" && kankei_namae_kana.val() !== undefined){
                if((kankei_namae_kana.val()).match(kana_pattern)){
                    success($(kankei_namae_kana.next('p.error_txt')));
                }else{
                    kankei_namae_kana.next('p.error_txt').text("お名前（フリガナ）はカタカナで入力してください。");
                    error($(kankei_namae_kana.next('p.error_txt')));
                }
            }else{
                kankei_namae_kana.next('p.error_txt').text("お名前（フリガナ）を入力してください。");
                error_text(kankei_namae_kana);
            };
        }
    }
    ...
    if($('#form-checkList-file_type').length){
        ...
        if(file_type == "ご家族でお車を増やす、または新規取得") {
            var nosya_elem = $('.form-nosya-m');
            if(nosya_m.val() == "" || nosya_m.val() == undefined){
                nosya_elem.children('p.error_txt').text("納車時期を選択してください。");
                error(nosya_elem.children('p.error_txt'));
            }else{
                success(nosya_elem.children('p.error_txt'));
            };
            // // ファイル添付を必須にしない
            // success(file_elem.find('p.error_txt1'));
            // success(file_elem.find('p.error_txt2'));
    ...
    // 紹介者
    if($('#form-checkList-introducer').length){
        error_check_radio(introducer,$('#form-checkList-introducer'));
        $('#form-checkList-introducer input:checked').each(function(){
            if($(this).val() == "あり"){
                error_text(name_full);
                name_full.next('p.error_txt').text("ご紹介者の方のお名前を入力してください。");
            } else {
                success($(name_full.next('p.error_txt')));
            }
        });
    }
